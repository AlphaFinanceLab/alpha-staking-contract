...
def test_single_stake(a, alice, worker, alpha, staking):
    ...
    assert staking.getStakeValue(
        alice) == stake_amt + reward_amt, 'incorrect reward to alice'
    assert alpha.balanceOf(staking) == stake_amt + \
        reward_amt, 'incorrect alpha in staking'
...
def test_single_stake_after_upgrade(a, alice, worker, alpha, upgraded_staking):
    ...
    assert upgraded_staking.totalAlpha() == stake_amt + \
        reward_amt, 'incorrect total alpha'
...
def test_many_stake_after_upgrade(a, alice, bob, worker, alpha, upgraded_staking):
    ...
    assert upgraded_staking.totalShare() == bob_stake_amt * stake_amt // (stake_amt +
    ...
    assert upgraded_staking.totalShare() == bob_stake_amt * stake_amt // (stake_amt +
                                                                          reward_amt) + stake_amt, 'incorrect total share'
    assert upgraded_staking.totalAlpha() == stake_amt + reward_amt + bob_stake_amt + \
        reward_amt_2, 'incorrect total alpha'
    assert upgraded_staking.getStakeValue(alice) == stake_amt + reward_amt + reward_amt_2 * \
        stake_amt // (upgraded_staking.totalShare()), 'incorrect alpha alice'
    assert upgraded_staking.getStakeValue(bob) == bob_stake_amt + reward_amt_2 * \
        (upgraded_staking.totalShare() -
         stake_amt) // upgraded_staking.totalShare(), 'incorrect bob alice'
