...
def test_pending_governor(staking):
    ...
    ) == '0x0000000000000000000000000000000000000000', 'incorrect pending governor'


def test_pending_governor_after_upgrade(upgraded_staking):
    assert upgraded_staking.pendingGovernor(
    ) == '0x0000000000000000000000000000000000000000', 'incorrect pending governor'
...
def test_set_governor_before_upgraded(staking, staking_v2, proxy_admin, deployer, alice):
    staking.setPendingGovernor(alice, {'from': deployer})
...
def test_set_governor_after_upgrade(upgraded_staking, deployer, alice):
    ...
    assert upgraded_staking.governor() == deployer, 'incorrect deployer governor after set'
    assert upgraded_staking.pendingGovernor(
    ) == alice, 'incorrect pending governor after set'
...
def test_accept_governor_before_upgrade(staking, staking_v2, proxy_admin, alice, deployer):
    staking.setPendingGovernor(alice, {'from': deployer})
    staking.acceptGovernor({'from': alice})
    proxy_admin.upgrade(staking, staking_v2)
    assert staking.governor() == alice, 'incorrect deployer governor after accept'
...
def test_accept_governor_after_upgrade(upgraded_staking, alice, deployer):
    ...
    assert upgraded_staking.governor() == alice, 'incorrect deployer governor after accept'
    assert upgraded_staking.pendingGovernor(
    ) == '0x0000000000000000000000000000000000000000', 'incorrect pending governor after accept'
...
def test_not_pending_accept_governor_before_upgrade(staking, staking_v2, proxy_admin, deployer, alice, bob):
    staking.setPendingGovernor(alice, {'from': deployer})
    proxy_admin.upgrade(staking, staking_v2)
    with brownie.reverts('acceptGovernor/not-pending'):
        staking.acceptGovernor({'from': bob})
...
def test_not_pending_accept_governor_after_upgrade(upgraded_staking, deployer, alice, bob):
    upgraded_staking.setPendingGovernor(alice, {'from': deployer})
    with brownie.reverts('acceptGovernor/not-pending'):
        upgraded_staking.acceptGovernor({'from': bob})
    assert upgraded_staking.pendingGovernor(
    ) == alice, 'incorrect pending governor after revert'
...
def test_set_governor_twice_before_upgrade(staking, staking_v2, proxy_admin, deployer, alice, bob):
    staking.setPendingGovernor(alice, {'from': deployer})
    assert staking.pendingGovernor() == alice, 'incorrect pending governor after set'
...
def test_set_governor_twice_after_upgrade(upgraded_staking, deployer, alice, bob):
    ...
    assert upgraded_staking.pendingGovernor(
    ) == bob, 'incorrect pending governor after set'
