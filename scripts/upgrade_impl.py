...
def main():
    ...
    staking = interface.IAny(
        TransparentUpgradeableProxyImpl.at("0x2aa297c3208bd98a9a477514d3c80ace570a6dee")
    )

    def test_upgrade(alpha, staking):
        network.gas_price(0)
        ...
        assert (
            bob_staking_v1_status == staking.STATUS_UNBONDING()
        ), "bob isn't unbonding"
        ...
        assert (
            charlie_after_share
            == charlie_stake_amount * total_share_v2 // total_alpha_v2
        ), "incorrect charlie share after stake"
        assert (
            charlie_after_unbond_time == 0
        ), "incorrect charlie unbond time after stake"
        assert (
            charlie_after_unbond_share == 0
        ), "incorrect charlie unbond share after stake"
        ...
        bob_withdraw_time = (
            bob_staking_v2_unbond_time + unbonding_duration_v2 - start_time
        )
        ...
        assert (
            alice_data_unbond_share == 0
        ), "incorrect alice unbond share after withdraw"

    # comment this function when upgrade contract
    test_upgrade(alpha, staking)
