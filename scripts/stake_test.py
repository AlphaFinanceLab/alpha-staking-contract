...
def main():
    ...
    assert (
        staking.totalAlpha() == stake_amt + reward_amt + bob_stake_amt + reward_amt_2
    ...
    assert (
        staking.getStakeValue(bob)
        == bob_stake_amt
        + reward_amt_2 * (staking.totalShare() - stake_amt) // staking.totalShare()
    ), "incorrect bob alice"
